...
`onCollision` | "push" | String, Function | <p>Defines what to do when the target element overflows the container element. The container element must be defined for this option to have any effect. Accepts a string identifying a built-in collision handling method or alternatively you can pass in a function and use this option as a callback function.</p><p>PoPo has two built-in collision methods: "push" and "push!". You can use them by providing this option with a string containing four method names separated by space. The four values each represent one side of the target element. The syntax is "left top right bottom". You can also skip collision on each side by using "none".</p><p>The "push" method tries to keep the targeted sides of the target element within the container element's boundaries. If you assign "push" method to the opposite sides the force of push will be equal on both sides. If you want to force one of the sides to be always pushed fully inside the container element's area, you can assign a forced push to that side with "push!" method.</p>
