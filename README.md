...
**container** | null | *Element* | <p>Defines an optional container element that is used for collision detection.</p><p>Alternatively you can define a coordinate by providing an array containining the coordinates and an element that is used as the parent element for the coordinates. The format is: [x-coordinate, y-coordinate, element]. The coordinates must be numbers (integers or floats) and the element must be a DOM element, the document object or the window object.</p>
